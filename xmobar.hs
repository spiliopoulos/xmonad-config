...
Config {
    ...
    commands = [
        ...
        Run Com "/bin/bash" ["-c", "/home/giannis/.xmonad/progress.sh $(/home/giannis/.xmonad/volume.sh) 10 0"] "volume" 1,
    ...
    sepChar = "%",
    alignSep = "}{",
    template = "%StdinReader% }{ %multicpu%  Vol: %volume%  %memory%   %swap%   %em1%   <fc=#FFFFCC>%date%</fc>   %KPAO%"
}
